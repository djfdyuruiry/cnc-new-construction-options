...
public partial class MainWindow : Window
{
  ...
  )
  {
    ...
    async Task<string> ConvertBinToIso(string cachePath, string binPath)
    {
    ...
    try
    {
      ...
      async Task<Stream?> GetStreamForSetupPackageFile(string name)
      {
      ...
      async Task ExtractFile(string fullFile, string? stripPrefix = null, string? outFile = null)
      {
        ...
        try
        {
          ...
          await using var sourceFileStream = iso.FileExists(fullFile)
            ...
            : (await GetStreamForSetupPackageFile(fullFile))!;
      ...
      {
        await ExtractFile(file, source.StripFilePrefix);
      }
      ...
      {
        await ExtractFile(file, source.StripFilePrefix, outFile: Path.Join(source.Faction, file));
      }
    }
    catch (Exception ex)
    {
  ...
  private async Task LoadData()
  {
    ...
    foreach (var source in sources)
    {
      ...
      {
        if (backgroundSet || !iso.FileExists("setup.bmp"))
        {
          return;
        }
        ...
        {
          Source = Bitmap.DecodeToHeight(fileStream, 480)
        };
