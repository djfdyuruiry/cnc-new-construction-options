...
The new save game format has some limitations due to game engine design:

- Cursor shapes are stored and read as shorts, they are no references to the occupy shapes in tiberiandawn/bdata.cpp (if these change, and then you load an old save, this may have undefined behavior)
- Saves from MEGAMAPS enabled builds and those not using MEGAMAPS are not compatible due to the cell layout and map binary format version
  - This also means you can't load a remaster save in the standalone engine build (But this is an obscure use case)
