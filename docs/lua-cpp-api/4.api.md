...
public:
    ...
    void Register_Functions(LuaEngine& engine) const override
    {
        With_Api_Namespace(engine, [&](auto& n) {
            // callable from lua as MyAPI.print(...)
...
6. **Native Modules**: Pass true parameter to base class for lua module registration (requires lua script `nco.<API_NAME>` to be available in package path)
...
- Review [SystemLuaApi](../../common/lua/system_luaapi.h) for a concrete example
- Examine [RulesLuaApi](../../common/lua/rules_luaapi.h) for another implementation
- Check [LoggingLuaApi](../../common/lua/logging_luaapi.h) for logging integration
