...
The long term goal is to add all Units, Infantry, Buildings etc. so you can define everything dynamically (graphics, special behavior etc).
...
A quickstart guide is here: [Scenario Lua Script Guide]()

Full documentation: [Lua API reference]()
...
Included with the Tiberian Dawn binary is `sc-psx-n64.mix`, which provides all the PS1 and N64 extra missions, as well as several N64 "FMVs".
...
The required mix files are provided with the Tiberian Dawn binary, no need to install separately.
