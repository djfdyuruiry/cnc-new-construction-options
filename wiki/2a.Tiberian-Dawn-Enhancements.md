...
distance of existing structures and walls.

[<img src="img/td-modern-walls.png" alt="Select all of same type" width="300"/>](img/td-modern-walls.png)

This enhancement is on by default, but can be toggled off using rules.ini, see the `ModernWallBuilding` rule in the [[Enhancements] section](2b.Tiberian-Dawn-Rules.md#enhancements).

- The max length of a wall section can be changed using the rule `[Enhancements] -> ModernWallMaxLength`
- By default, the wall pieces are at full cost. This can be changed in the `[Enhancements] -> ModernWallFullCost` rule
- This enhancement respects the rules `[Enhancements] -> MaxBuildDistance` and `[Game.Map] -> AllowBuildingBesideWalls`
