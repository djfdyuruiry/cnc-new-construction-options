...
The original game engine uses a somewhat clunky algorithm for moving units from point A to B, moving around obstecles like ridges, walls and trees. Often units will not do what you would expect, for example going towards a ridge then round it rather than just going round it immediately.

The A* path finding feature fixes these issues, making unit movement more efficent and direct. Note that will affect both human and AI players, so AI units in a single player mission might arrive sooner than expected.
