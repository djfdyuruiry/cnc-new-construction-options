...
- [Save Game Screenshots](#save-game-screenshots)
- [JSON Save Games](#json-save-games)

### Additional Content

- [Mission Select Menu](#mission-select-menu)
- [Improved Skirmish Setup](#improved-skirmish-setup)
...
An [in-game editor](2b.Tiberian-Dawn-Rules.md#rules-editor) for game rules has been created to make modding easier:

  [<img src="img/td-rules-editor.png" width="400">](img/td-rules-editor.png)
...
You can hold `CTRL` to force placing a single wall piece instead of straight lines.
...
Save game load/save/delete dialogs now include a screenshot and a mission description to help you find the right save.

[<img src="img/td-save-game-screenshots.png" alt="Select all of same type" width="640"/>](img/td-save-game-screenshots.png)
...
[<img src="img/td-skirmish-screen.png" alt="Skirmish Setup" width="640"/>](img/td-skirmish-screen.png)

The skirmish screen (originally copied from Red Alert) has been improved to include a map preview and more options:

- AI Player Difficulty
- AI Player Sides
- Tiberium Growth Factor 
...
[<img src="img/td-scenario-editor.webp" alt="Skirmish Setup" width="640"/>](img/td-scenario-editor.webp)

Use the internal Scenario Editor developed by Westwood Studios when building C&C! This has been enhanced with a new sidebar
for viewing the minimap and placing map objects. A rules editor is built in to all setting custom rules for the scenario.

**Features:**

- Accessing Editor from main menu (Select `Scenario Editor`)
- Creating a new Scenario
- Opening existing Scenarios using Mission Select menu
- Visualizing map boundaries and editing terrain outside the map to influence reinforcements
- Adding units/terrain/buildings etc.
- Adding triggers for objects and map cells
- Placing waypoints
- Editing/Deleting triggers and waypoints
- Setting up AI base (buildings not present at scenario start are shown faded)
- Setup Lua triggers and Lua script path for scenario
- Saving Scenario (files are written to user path)
- Playing the Scenario directly from the Editor (you **must** save when prompted)
- Hi-Res dialogs

**Known Issues:**

- Main menu `Scenario Editor` text and editor dialogs are all hardcoded in English
- Changing theater during editing can cause graphical issues until game is restarted
