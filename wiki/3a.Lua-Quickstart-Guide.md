...
Scripts control mission behavior and are loaded by the game engine on mission start.
...
Simply typing 'text editor' into the system menu on most other operating systems will give you a similar app.

## Background: How The Game Loads Scripts

When you start a mission, the engine looks for Lua files in a specific order:
...
Team types are groups of units that can used as reinforcements or as a group the AI will build automatically. If the last unit in the group is a transport (APC, Chinook, Hover Craft) then all other units will be inside it; for example 5 grenadiers inside an APC

The names of units and infantry correspond to those found in scenario INI files.

The definition in Lua above uses the original syntax found in scenario INI files:
...
Event.handlers.myLuaEvent = function(triggerName)
  -- give GDI $$$$
  Scenario.houses.GoodGuy.giveMoney(5000)

  -- show an on-screen message to the player
  if not System.isRemasteredMod then
...
- You will see the message printed in the top left of your screen after 5 seconds
  - **Note:** Messages will not be shown in the Remastered mod
- You will see that 5000 credits have been given to you
...
> **NOTE:** Lua console is not available in the Remastered Mod
...
<img src="img/lua-console.png" alt="vision" width="600"/>
...
To view the log file, follow the steps for your Operating System below:

- **Windows**: Press Windows Key + R, type `%APPDATA%\nco` and hit enter
- **macOS**: Open Finder, choose Go > Go to Folder and enter `~/Library/Application Support/nco`
- **Linux**: In a terminal, run `xdg-open ~/.config/nco`

You will find a `nco-td` file in the folder you just opened, open this in a text editor and scan through the entries to find any errors.

> **Note**: If you are running NCO as a mod for the Remastered Collection the log file will be named `TIBERIANDAWN.DLL`

---

Here is an example of what to look for in the log file. If I had a Lua syntax error (invalid code):
...
I would see a message similar to the below in the log file:

```json
{ "time": "11:17:31.563", "level": "E", "message": "__debug.lua:8: syntax error near '`'", "logger": "nco", "at": "debug.cpp:568", "in": "auto Debug_Key(unsigned int)::(lambda)::operator()(const auto &) const [r:auto = LuaResult]", "date": "15-05-26", "process": 558022, "thread": 558024 }
```

Note the `"message"` field, which points out the line in the file and the error.
