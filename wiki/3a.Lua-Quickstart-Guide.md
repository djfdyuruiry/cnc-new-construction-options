...
Lua scripts should be placed in the `lua` folder where Tiberian Dawn is installed. 

### Finding the Lua Folder

If you installed NCO using the Launcher app (recommended), follow the steps for your Operating System below:

- **Windows**: Press Windows Key + R, type `%LOCALAPPDATA%\nco\td\lua` and hit enter
- **macOS**: Open Finder, choose Go > Go to Folder and enter `~/Library/Application Support/Vanilla-Conquer/vanillatd/lua`
- **Linux**: In a terminal, run `xdg-open ~/.local/share/nco/td/lua`
