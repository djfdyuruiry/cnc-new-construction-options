...
The `Logging` object is available to scripts, this contains functions that are used to write messages to the game log file and the terminal (if you started the game from a terminal - PowerShell, Bash etc.)

> For help on viewing logs, see: [Lua Debugging](3a.Lua-Quickstart-Guide#debugging)
...
- `System.openGameFile` looks for files in the folder where the game is installed. (**Note: this directory is typically read-only**)

- `System.openUserFile` looks for files in the user directory. Your user directory depends on your Operating System:
...
```lua
-- open my-data.txt
local myData, fileError = System.openUserFile("my-data.txt", "w")
...
If you create a `__debug.lua` file inside the Lua folder, it can be executed on demand whilst playing a mission. This is useful to test out new pieces of code or just figure out why something doesn't work.

> Help on finding the Lua folder can be found here: [Lua Debugging](3a.Lua-Quickstart-Guide#finding-the-lua-folder)

You need to start the game in 'cheat' mode to enable this:

  - In the game folder, find `nco-td-cheats` and run it instead of the main game
    - This might be `nco-td-cheats.bat` or `nco-td-cheats.sh` depending on your OS
  - When in a mission you can press `F5` to trigger the debug lua script
  - View the game log file in the game folder to see if any errors occurred (For help on viewing logs, see: [Lua Debugging](https://github.com/djfdyuruiry/cnc-new-construction-options/wiki/3a.Lua-Quickstart-Guide#debugging))
  - The script is reloaded every time you press `F5`, so you can edit it whilst the game is running

## Test Lua Outside the Game

> This is only available to Windows and Linux users, macos releases don't include this feature

For more complex scripts you might want to test the script outside the game. NCO includes a Lua interpreter with the game, in the `lua` folder
