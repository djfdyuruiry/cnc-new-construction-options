...
2. Copy and paste the below and hit enter:
    ```pwsh
    powershell -ExecutionPolicy Bypass -Command "iex (iwr ""https://raw.githubusercontent.com/djfdyuruiry/cnc-new-construction-options/refs/heads/main/scripts/remaster/install.ps1"" | Select -ExpandProperty Content)"
    ```
...
3. Paste the below and hit enter:
    ```shell
    curl -fsSL "https://raw.githubusercontent.com/djfdyuruiry/cnc-new-construction-options/refs/heads/main/scripts/remaster/install.sh" | bash
    ```
...
1. Open `VC_redist.x86` and install
1. Download the mod zip files from the latest release [here](https://github.com/djfdyuruiry/cnc-new-construction-options/releases/tag/latest)
![Launcher Downloads](img/remaster-download.png)
1. Press Windows Key + R
1. Type `%USERPROFILE%\Documents\CnCRemastered\Mods` and hit enter
1. Unzip `nco-remaster-td-mod-msvc` into `Tiberian_Dawn` (Create the `Tiberian_Dawn` folder if it doesn't already exist)
1. Unzip `nco-remaster-ra-mod-msvc` into `Red_Alert` (Create the `Red_Alert` folder if it doesn't already exist)
